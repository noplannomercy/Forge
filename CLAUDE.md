...
| 경로 | 역할 |
|------|------|
| `app.py` | FastAPI 엔드포인트 (/convert, /result/{job_id}, /batch, /health) + ?route= 파라미터 |
| `router.py` | 포맷 감지 + 경로 결정 (extract vs vlm) + route_override 지원 |
| `vlm.py` | VLM semantic 배치 클라이언트 (process_batch + Semaphore + retry) |
| `job_store.py` | JobStore ABC + InMemoryJobStore (+ PostgresJobStore 예정) |
| `worker.py` | 비동기 변환 워커 (extract/vlm 라우팅 + PPTX→PDF 파이프라인) |
| `config.py` | 환경변수 로드 (VLM, VLM_BATCH_SIZE, DATABASE_URL 등) |
...
# 예상: 77+ passed (v2 기준, .env로 인한 test_config_defaults 1개 실패는 허용)
