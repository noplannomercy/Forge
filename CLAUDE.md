...
| C2 | DOCX/XLSX/HWPX는 extract, PPTX/이미지PDF/이미지는 VLM semantic | PPTX는 이미지 위주라 extract 무의미. DOCX/XLSX/HWPX는 텍스트+표 위주라 extract가 쓸만함. |
...
| 경로 | 역할 |
|------|------|
| `app.py` | FastAPI 엔드포인트 (/convert, /result, /batch, /health) + admin 라우터 마운트 |
...
| `schema.sql` | PostgreSQL DDL (forge_jobs, forge_vlm_logs, forge_prompts, 인덱스) |
...
| `extractors/hwpx.py` | HWPX 텍스트+표 → md (ZIP+XML 파싱) |
...
| docs/superpowers/specs/2026-04-09-forge-prompt-externalize-design.md | 프롬프트 외부화 스펙 |
| docs/superpowers/plans/2026-04-09-forge-prompt-externalize.md | 프롬프트 외부화 플랜 (6 Task) |
| docs/superpowers/specs/2026-04-09-forge-callback-design.md | callback URL 스펙 |
